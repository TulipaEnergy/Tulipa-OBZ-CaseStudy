...
For this tutorial, we'll use the OBZ data. Check <https://github.com/TulipaEnergy/Tulipa-OBZ-CaseStudy> for more information.
...
TulipaIO.read_csv_folder(
    ...
    skip = 1, # Data in this folder has an extra header line for the types, which we'll ignore
)
TulipaIO.read_csv_folder(
    connection, 
...
subtable = DuckDB.query(
    ...
    WHERE 
        ...
        AND year=$year
        AND timestep <= 72 -- Just 72 hours of plot
    ORDER BY timestep
    ",
)
...
PS. We chose to define our `clustering_params` a `NamedTuple` in Julia, but that is completely optional and you can use whatever structure suits your case.
...
TulipaClustering.cluster!(
    ...
    clustering_params.num_rep_periods;
...
Now the ~hard~ fun part starts. We need to create specific tables for Tulipa using our current tables.
...
We have to define a minimum of column for each table, and then the remaining columns will be filled with defaults.
The same is true for missing values in these columns.
...
!!! info "If you skipped ahead"
    If you skipped ahead and have errors here, check out some of the previous steps.
    Notably, populating with defaults helps solve many issues with missing data and wrong types in the columns.

```@example obz
energy_problem = TEM.EnergyProblem(connection)
```

Now we have all tables. Below is the count of tables in general. Notice the `main` schema, which is the default schema for the tables.

```@example obz
nice_query(
    "SELECT schema_name, COUNT(*) as num_tables,
    FROM duckdb_tables()
...
Now every variables indices table has a column `solution`, and every constraint has additional columns `dual_*`, depending on the constraints name.
